list.Set("NPC", "npc_uvspecial", {
	Name = "#uv.npc.5special",
	Class = "npc_uvspecial",
	Category = "#uv.unitvehicles"
})

AddCSLuaFile("npc_uvspecial.lua")
...
if SERVER then	
	...
	function ENT:OnRemove()
		...
		if self.v and not self.v.disengaging and self.metwithenemy and not UVResourcePointsRefreshing and UVGlobalPursuit.ResourcePoints > 1 and not self.v.rhino and not UVOneCommanderActive and not self.roadblocking then
		...
		if not self:Validate(self.e) then --If it doesn't have an enemy.
			...
			if useDirectDriveBranch then
				...
				if (not self.formationpoint or enemyvelocity <= UVBustSpeed * 30)
					...
					elseif (not self.driveinfront or obstaclesNearbySide) then
			...
			if not self.metwithenemy and edistSqr < 25000000 and self:StraightToTarget(self.e) then
				...
				if CurTime() > self.bountytimer + botimeout then
					...
					if MathAggressive == 1 then
						if not self.aggressive and UVTargeting then
							...
							if Chatter:GetBool() and self:StraightToTarget(self.e) and not UVCalm then
					...
					if Chatter:GetBool() and enemyvelocity > 100000 and self:StraightToTarget(self.e) and MathAggressive ~= 1 then
	...
	function ENT:Initialize()
		...
		if DriverModel:GetBool() then
			local selectedDriverModel = self.v.rhino and GetConVar("unitvehicle_unit_rhino_drivermodel"):GetString() or GetConVar("unitvehicle_unit_special_drivermodel"):GetString()
		...
		if not UVTargeting then self.v:EmitSound( "plats/tram_hit1.wav" ) end
